...
export const async_actions = {
    ...
    [`fetch_all_data`]: createAsyncThunk(
        ...
        async (data, thunkAPI) => {
            ...
            let state = thunkAPI.getState().photo_gallery_categorie;
            let qparams = {
                page: state[`page`],
    ...
    [`store_${store_prefix}`]: createAsyncThunk(
        `photo_gallery_categories/store_${store_prefix}`,
        async (form_data, thunkAPI) => {
            ...
            try {
                ...
                // console.log('from store user roles',error);
    ...
    [`edit_${store_prefix}`]: createAsyncThunk(
        `photo_gallery_categories/edit_${store_prefix}`,
        async (form_data, thunkAPI) => {
            // console.log(thunkAPI);
            console.log('hoiche');
            try {
    ...
    [`details_${store_prefix}`]: createAsyncThunk(
        `photo_gallery_categories/details_${store_prefix}`,
        async (id, thunkAPI) => {
            console.log(id);
            try {
    ...
    [`fetch_my_data`]: createAsyncThunk(
        ...
        async (params = 0, thunkAPI) => {
            // console.log(thunkAPI);
            const response = await axios.get(`${app_config.api_endpoint}/user-roles`);
